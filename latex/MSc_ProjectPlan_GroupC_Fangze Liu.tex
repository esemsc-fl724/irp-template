...
Due to inherent technical limitations, large language models (LLMs) cannot directly communicate with external tools. To solve this problem, OpenAI introduced Function Calling in March 2023. This technique enables LLMs to call external tools by creating an intermediary function that bridges the model’s request and the external API, so that the model has the ability to call external tools. Function Calling is an important foundation to promote the evolution of the LLMs from a dialogue tool to an AI Agent,  which are capable of autonomously perceiving the environment, making decisions, and taking actions.
...
To address this challenge, Anthropic released the Model Context Protocol (MCP) in November 2024, introducing a standardized and distributed architecture consisting of a Client and a Server (Figure 2). With a unified specification, the development efficiency can be greatly improved.

Overall, MCP provides standardised interfaces, support for distributed and asynchronous execution, and a good developer ecosystem, and fits the real-world business needs. In this project, we plan to explore MCP-based calling modules to improve interface compatibility and response performance, and lay the foundation for multi-model collaboration and complex task scheduling.

% Figure
\begin{figure}[H]
  \centering
  \fcolorbox{white}{white}{
    \includegraphics[width=0.8\textwidth]{figures/MCP.png}
  }
  \caption{MCP}
  \label{fig:mcp}
\end{figure}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item Optionally extend support to historical weather data queries or knowledge-augmented generation (e.g., RAG~\cite{GaoYunfan2023}). Further literature review is required to assess feasibility.
  \end{itemize}
\end{enumerate}
...
\hline
MCP and Extension Integration & 7–8 & Integrate MCP protocol, design decoupled client-server architecture, implement exploratory extension\\
\hline
...
\begin{ganttchart}[
    ...
    \ganttbluebar{MCP Protocol and Extension}{2025-07-14}{2025-07-27}\\ [grid]
